...
public final class GitlabMrCommentBuilder {
    ...
    public static String build(MrCodeReviewEntity review) {
        ...
        sb.append("---\n");
        sb.append("*由 VCS 平台自動產生 · reviewId=").append(review.getId());
        if (review.getDiffHash() != null) {
            sb.append(" · diff=").append(review.getDiffHash().substring(0, Math.min(8, review.getDiffHash().length())));
        }
        sb.append("*");
