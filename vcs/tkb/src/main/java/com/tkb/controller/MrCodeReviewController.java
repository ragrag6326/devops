...
public class MrCodeReviewController {
    ...
    public Result<MrReviewScanResultDTO> scanProject(
            @Parameter(description = "專案名稱", example = "go_nuxt")
    ...
    @Operation(summary = "N8N 回寫審核結果（Webhook Callback）")
    @PostMapping("/callback")
    public Result<String> callback(@RequestBody MrReviewCallbackDTO callback) {
        log.info("收到 N8N callback, reviewId={}", callback != null ? callback.getReviewId() : null);
        mrCodeReviewService.handleCallback(callback);
        return Result.success("ok");
    }

    @Operation(summary = "GitLab MR Webhook（Merge Request 事件即時觸發審核）")
    ...
    public Result<String> gitlabWebhook(
        ...
        String result = mrCodeReviewService.handleGitlabWebhook(payload, gitlabToken);
    ...
    @Operation(summary = "依專案與 MR IID 查詢最新審核結果")
    @GetMapping("/detail")
    public Result<MrCodeReviewEntity> detail(
            @RequestParam String projectName,
            @RequestParam Integer mrIid
    ) {
        MrCodeReviewEntity entity = mrCodeReviewService.getByProjectAndIid(projectName, mrIid);
