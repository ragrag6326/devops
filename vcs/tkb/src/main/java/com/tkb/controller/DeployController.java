...
@Tag(name = "4.0.0 部屬狀態標記", description = "提供部屬中、成功、失敗、回滾的標記")
@Slf4j
...
public class DeployController {
    ...
    @Operation(summary = "4.0.1 部屬中", description = "將該版號印上部屬標示 (Deploying)")
    @PostMapping("/deploying")
    public Result<Long> deploying(
            @Parameter(description = "部屬版號", required = true, example = "1.0.0")
            @RequestBody DeployDTO dto) {
        Long id = service.markDeploying(dto);
        return Result.success(id);
    }

    @Operation(summary = "4.0.2 部屬成功", description = "將該版號標記為成功 (Success)")
    @PostMapping("/success")
    ...
    @Operation(summary = "4.0.3 部屬失敗", description = "將該版號標記為失敗 (Fail)，通常由 Jenkins Pipeline 觸發")
    @PostMapping("/fail")
    ...
    @Operation(summary = "4.0.4 版本回滾", description = "標記該版號為已回滾 (Rollback)")
